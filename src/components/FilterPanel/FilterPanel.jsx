...
const FilterPanel = ({
  ...
  // Only show device types that actually exist for the selected brand in the database.
  // If no brand is selected, show every type that exists across all brands.
  const availableTypes = selectedBrand
    ? brandsWithTypes.find((b) => b._id === selectedBrand)?.types || []
    : [...new Set(brandsWithTypes.flatMap((b) => b.types))];
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
      ...
      <div className="mb-5">
        ...
        <div className="flex flex-wrap gap-2">
          {/* All 17 brands always shown, from the constant list — not just brands with existing products */}
          {BRANDS.map((brandName) => (
            <button
              key={brandName}
              onClick={() =>
                onBrandChange(selectedBrand === brandName ? "" : brandName)
              }
              className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${
      ...
      <div>
        ...
        {availableTypes.length === 0 ? (
          <p className="text-gray-500 text-xs">
            No products yet for this brand.
          </p>
        ) : (
          <div className="flex flex-wrap gap-2">
            {availableTypes.map((t) => (
